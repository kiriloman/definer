from definer_client import get_definition
...
def define():
    while True:
        ...
        data, valid = __validate_data(input_str)
        ...
        defined_word = get_definition(
            word=word.strip(),
            lang=lang.strip(),
        )

        if isinstance(defined_word, dict):
            print(__format_definition(defined_word))
        else:
            print(defined_word)


def __validate_data(data: str) -> tuple:
...
def __format_definition(data: dict) -> str:
